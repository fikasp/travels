...
def process_gpx_file(gpx_path):
    """
    Worker executed in a parallel process.
    Returns list of tuples: (print_message, formatted_entry).
    We return both the message (for identical prints) and the formatted JS entry,
    so that printing is done in the main process (keeps console output consistent).
    """
    results = []
    try:
        extracted = extract_data(gpx_path)
        for name, range_, activity, coords in extracted:
            ...
            if range_ == "POLSKA":
                ...
                msg = f"🟥❌ {icon} {name} -> {range_}"

            entry = format_route_entry(name, icon, range_, activity, year, length, coords)
            results.append((msg, entry))
    except Exception as e:
        # If parsing inside worker fails, return an error message tuple
        results.append((f"❌ Error processing {gpx_path}: {e}", None))
    return results
...
def main():
    ...
    try:
        ...
        # ignore if platform doesn't support or file missing
        pass

    # Collect GPX files, excluding certain folders (same logic as original)
    gpx_files = [
        f for f in base_folder.rglob('*.gpx')
        if "Stoki" not in f.parts
    ]

    new_entries = []

    if not gpx_files:
        print("⚠️ No GPX files found.")
    else:
        # Use multiprocessing Pool to process files in parallel
        # Each worker returns a list of (msg, entry) tuples; we flatten them.
        with Pool(cpu_count()) as pool:
            results = pool.map(process_gpx_file, gpx_files)

        # Flatten the list-of-lists and separate messages from entries
        flattened = [item for sub in results for item in sub]

        # Print messages (to preserve original behavior of printing per-route logs)
        for msg, entry in flattened:
            print(msg)

        # Collect formatted entries, ignoring None entries
        new_entries = [entry for msg, entry in flattened if entry]
